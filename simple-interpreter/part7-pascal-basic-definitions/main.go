...
/*
 * part7: add support of pascal basic definition structures.
 */

// -*- Lexer -*-

const (
	tokLiteralInteger = iota
	tokLiteralReal
	tokPlus
	tokMinus
	tokMul
...
func (p *parser) consumeTerm() (node visitable, err error) {
	...
	for p.lexer.currentToken().kind == tokMul || p.lexer.currentToken().kind == tokDivInteger {
...
func (p *parser) consumeVariable() (node visitable, err error) {
	...
	if err = p.lexer.eatToken(tokSymbol); err != nil {
		return nil, err
	}
	return
}
...
func (p *parser) consumeStatement() (node visitable, err error) {
	...
	} else if p.lexer.currentToken().kind == tokSymbol {
...
func (e *evaluator) visitBinOp(node *nodeBinOp) (result int, err error) {
	...
	if opKind != tokPlus && opKind != tokMinus && opKind != tokMul && opKind != tokDivInteger {
